...
La première colonne à ajouter indiquera, pour chaque roman, le nombre de caractères (y compris le nombre d'espaces); la deuxième, le nombre de mots; la troisième, le nombre de phrases; la quatrième, le genre de l'ouvrage (roman/essai). Les données catégorielles demanderaient un développement en soi. Disons pour aller vite que ce sont des données qui se présentent au programmeur comme des chaînes de caractères, mais R les appréhende sous la forme de nombres entiers. On donne cette structure à la donnée lorsque sa variété est limitée et répétitive. La fonction utilisée pour créer un tel type de données est `factor()` et la transformation de chaînes de caractères en données catégorielles passe par la fonction `as.factor()`. On verra ci-dessous, une fois les données créées, l'implication de cette transformation.
