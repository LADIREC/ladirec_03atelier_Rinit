...
if(!"rmdformats" %in% rownames(installed.packages())) {install.packages("rmdformats")}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE)
extensions = c("knitr", "ggplot2", "dplyr", "rmdformats", "stringr", "gutenbergr", "reshape2")
...
Pour ajouter une colonne au tableau, on crée un vecteur et on l'ajoute au tableau à l'aide de la fonction `cbind()`. On peut imposer un nouvel ordre aux colonnes en insérant, en position `j` de l'indexation, un vecteur composé des numéros de colonnes.

```{r ajout-colonne}

initiales_auteur <- c("AH","NB", "JB", "MD")

mes_oeuvres_df <- cbind(mes_oeuvres_df, initiales_auteur)
...
rm(list=setdiff(ls(),c("tableau_synthese", "mes_oeuvres_df")))
...
library(stringr) # On réactive l'extension `stringr`, au cas où nous ne l'ayons pas fait déjà.
...
Comme nous avons fait différents calculs à partir des textes, nous allons pouvoir composer un jeu de données qui les exclut, cela allégera d'autant la manipulation de la structure. On ne supprime pas la première structure, on la reproduit sans la volumineuse variable `texte`.
```{r visualisation}
library(reshape2)
library(ggplot2)
