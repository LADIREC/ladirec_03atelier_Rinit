...
# Combien d'auteurs uniques de langue française dans le PG?
...
ggplot(tableau_synthese[, c(1,3:4)], aes(x = reorder(titre, nbre_mots), y = nbre_phrases, size = nbre_mots)) +
  geom_point(stat = "identity")+
  xlab("Titres")+
  ylab("Nombre de phrases")


ggplot(melt(tableau_synthese[, c(1,3:4)], id.vars="titre"), aes(x=reorder(titre, value), y=value, fill= variable))+
...
## Supplément: l'annotation automatique

```{r}
# library(udpipe)
# library(data.table)
# 
# model <- udpipe::udpipe_download_model(language = "french-gsd")
# model_gsd <- udpipe_load_model("french-gsd-ud-2.5-191206.udpipe")
# 
# RachildeAnnotee <- udpipe::udpipe_annotate(model_gsd,
#                                            tableau_synthese$texte,
#                                            doc_id = tableau_synthese$titre) %>% 
#   as.data.table()
```
