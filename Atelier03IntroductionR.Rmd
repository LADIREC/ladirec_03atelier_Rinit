...
* Manipulation de listes et de tableaux de données
* Les structures de contrôle `if`, `for` et `while`
* Manipulation libre d'un tableau formé de gazouillis
...
On a appris à manipuler les vecteur dans le premier atelier, mais on a laissé de côté celle des listes. Créons une courte liste comprenant trois vecteurs ayant chacun trois éléments.
...
Si je souhaite effectuer une opération sur chacun des éléments de la liste (pour autant que cela soit autorisé par le type de données qu'ils contiennent et par la fonction utilisée), je pourrais recourir à la fonction d'itération `lapply()`, qui prend au minimum deux arguments: `x`, soit un vecteur ou une liste, ainsi qu'une fonction qui sera appliquée à chaque élément de `x`. 
...
Créons maintenant un tableau de données à partir des éléments de la liste en mémoire. Chaque élément de la liste, trois vecteurs, seront assemblés à la verticale et deviendront les colonnes du tableau.

```{r creation-data-frame}
(mes_oeuvres_df <- data.frame(mes_oeuvres_l))
```

Dans un ableau de données, chaque colonne constitue un vecteur indépendant des autres et composé de données **d'un seul type**. On peut cependant manipuler ces différents vecteurs et même produire de nouvelles colonnes à l'aide de ces manipulations. Ajoutons tout d'abord une colonne `prenom` au tableau puis assemblons, dans une nouvelle colonne, les éléments de cette colonne et ceux de la colonne auteurs. Et tant qu'à faire, profitons-en pour modifier le nom de cette colonne actuellement appelée `auteurs`.


```{r}
mes_oeuvres_df$prenom <- c("Louis-Sébastien", "Germaine de", "Victor", "Émile") # On crée ici une colonne appelée "prenom" et composée de quatre éléments.

mes_oeuvres_df$nom_prenom <- paste(mes_oeuvres_df$auteurs, mes_oeuvres_df$prenom, sep = ", ") # On crée ici une colonne appelée "nom_prenom" et composée des valeurs de deux colonnes existantes.

mes_oeuvres_df <- mes_oeuvres_df[, c(5,1,4,2,3)] # Les nouvelles colonnes sont toujours placées à la droite du tableau, par défaut. On peut réordonner nos colonnes en fournissant un vecteur en position `j` dans les crochets d'indexation.

names(mes_oeuvres_df)[names(mes_oeuvres_df)=="auteurs"] <- "nom" # Ici, on indexe le nom de la colonne "auteurs" afin d'écraser précisément ce nom par un autre.
...
Ajoutons une nouvelle colonne à notre tableau, réordonnons l'ordre des colonnes et donnons de nouveaux noms à certaines d'entre elles.

```{r}
mes_oeuvres_df <- mes_oeuvres_df %>%
  mutate(., abbreviation=paste(str_extract(prenom, pattern="[[:upper:]]"),
                               str_extract(nom, pattern="[[:upper:]]"), sep = " ")) %>%
  select(nom, prenom, abbreviation, oeuvres, annees_pub) %>%
  rename(., patronyme=nom)

# Trois opérations en chaîne:
# 1. Création d'une nouvelle colonne avec la fonction `dplyr::mutate()`;
# 2. On impose un nouvel ordre avec la fonction `dplyr::select()`;
# 3. On donne un nouveau nom à la colonne `nom`.

```


## Les structures de contrôle
Il arrive qu'on ne souhaite appliquer une fonction ou une série d'opérations sur une variable ou un ensemble d'observations que si certaines conditions sont remplies, ou encore tant qu'un résultat ne sera pas obtenu, ou encore un nombre de fois prédéterminé. C'est alors qu'on a besoin des structures de contrôle.

### L'instruction if()

Nous avons déjà croisé une telle syntaxe lorsqu'on souhaitait vérifier si certaines extensions étaient installées dans nos dossiers R :

```{r condition-if}
if(!"ggplot2" %in% rownames(installed.packages())) {install.packages("ggplot2")}
```

La ligne de code ci-dessus doit se lire ainsi: « si la condition indiquée dans la parenthèse suivant `if` n'est pas vraie, exécute la commande insérée à la suite, dans les accolades. » `installed.packages()` est une matrice dont les noms de lignes correspondent aux extensions installées dans vos dossiers. Ainsi, la ligne de code ci-dessus indique à R que si l'extension `ggplot2` *n'est pas* incluse dans les noms de lignes de la matrice appelée par la fonction `installed.packages()`, il devra installer cette extension. 

L'instruction `if()` appelle donc la vérification d'une assertion contenue dans une parenthèse et, selon le résultat du test, elle exécutera ou non la ou les commandes qui suivent. On pourrait par exemple utiliser `if()` dans la création d'une nouvelle variable dans un tableau de données. Pour le montrer, créons pour commencer un tableau très simple :
...
```{r}
if(!"Nelly Arcan" %in% auteures_quebecoises$nom) {             
  print("Non, Nelly Arcan ne figure pas dans le tableau.")      
} else if (!"Folle" %in% auteures_quebecoises$oeuvre){         
  auteures_quebecoises <- rbind(auteures_quebecoises, list("Nelly Arcan", "Folle", 2004L, "roman"))     
  print("Le roman Folle fait maintenant partie du tableau.")    
}
```
...
Que s'est-il passé? Sans doute pas ce que nous espérions! R a vérifié la première assertion de la structure conditionnelle et a conclu qu'elle était fausse; il est donc passé à la suivante, et ainsi de suite jusqu'à ce qu'il trouve une assertion vraie ou une commande `else`. Il a dès lors appliqué au nouveau vecteur formant la colonne *periode* la même commande. Comme on le devine, cela n'est pas très utile. Ce qu'il faut, c'est que chaque observation de la variable `annee` fasse l'objet d'une évaluation indépendante et que le résultat de la structure conditionnelle soit renvoyé comme valeur du nouveau vecteur *periode*. Il y a au moins deux manières de faire cela sans recourir à des extensions spécialisées dans la manipulation de données comme `dplyr`. La première consiste à utiliser une boucle de type `for`.
...
if(reponse == tolower("oui")) {
  ...
  auteure_temp <- readline(prompt = "Écris le nom d'une auteure (ex.: Elisabeth Bégon):  ")
  oeuvre_temp <- readline(prompt = "Donne le titre de l'une de ses oeuvres (ex.: Lettres au cher fils): ")
  annee_temp <- readline(prompt = "Indique l'année de publication de cette oeuvre (ex.: 1748):  ")
  genre_temp <- readline(prompt = "Précise le genre de cette oeuvre (ex.: lettres):  ")
...
auteures_quebecoises <- auteures_quebecoises[order(auteures_quebecoises$annee, decreasing = TRUE),]
(auteures_quebecoises)
```
...
```{r}
for(i in 1 : nrow(auteures_quebecoises)){
  auteures_quebecoises$type[i] <- ifelse(test = auteures_quebecoises$genre[i] == "poésie",
                                         yes = "vers",
                                         no ="prose")
}

(auteures_quebecoises <- auteures_quebecoises[,c(1,2,3,5,4,6)]) # On réordonne ici les colonnes pour une présentation plus adéquate
...
litt_twitter <- search_tweets("#Littérature", n=1000, lang = "fr", include_rts = FALSE, type = "mixed")
